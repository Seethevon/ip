    ...
    public static void handleCommand(String input, TaskList tasks, Ui ui) {
        try {
            if (input.equals("list")) {
                ...
                ui.showMessage("Good job! Bibi.Bibi.Task marked as done:\n" + tasks.getTask(taskNumber));
            } else if (input.startsWith("unmark ")) {
                int taskNumber = Integer.parseInt(input.substring(7).trim()) - 1;
                tasks.unmarkTask(taskNumber);
                ui.showMessage("Bibi.Bibi.Task marked as not done:\n" + tasks.getTask(taskNumber));
            } else if (input.startsWith("todo ")) {
                tasks.addTodo(input.substring(5).trim());
                ui.showMessage("Added a Bibi.Bibi.Todo:\n" + tasks.getLastTask());
            } else if (input.startsWith("deadline ")) {
                String[] parts = input.substring(9).split(" /by ");
                tasks.addDeadline(parts[0].trim(), parts[1].trim());
                ui.showMessage("Added a Bibi.Bibi.Deadline:\n" + tasks.getLastTask());
            } else if (input.startsWith("event ")) {
                String[] parts = input.substring(6).split(" /from | /to ");
                tasks.addEvent(parts[0].trim(), parts[1].trim(), parts[2].trim());
                ui.showMessage("Added an Bibi.Bibi.Event:\n" + tasks.getLastTask());
